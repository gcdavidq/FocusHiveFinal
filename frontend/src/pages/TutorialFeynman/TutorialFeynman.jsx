...
const EMPTY_FORM = { tema: '', desarrollo: '', informacion: '', reescritura: '' };

/** Lee el borrador del tutorial guardado en este navegador (sin cuenta). */
function loadDraft() {
  try {
    const saved = localStorage.getItem('feynmanTutorial');
...
function TutorialFeynman() {
  ...
  // Borrador local: este tutorial es público y no requiere cuenta
  ...
  const handleReset = async () => {
    const ok = await ui.confirm({ title: 'Borrar progreso', message: 'Se eliminará el borrador guardado en este navegador.', confirmText: 'Borrar', danger: true });
    if (!ok) return;
